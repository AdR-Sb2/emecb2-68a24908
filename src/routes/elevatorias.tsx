...
function ElevatoriasPage() {
  ...
  const calcularCompletudes = async (elevs: Elevatoria[]) => {
    ...
    for (const elev of elevs) {
      ...
      for (const field of elevFields) {
        elevTotal++;
        if (
          elev[field as keyof Elevatoria] !== null &&
          elev[field as keyof Elevatoria] !== "" &&
          elev[field as keyof Elevatoria] !== undefined
        ) {
          elevPreenchidos++;
        }
      }
      ...
      for (let i = 0; i < tabs.length; i++) {
        ...
        const tabData =
          resultsByTable
            .get(tabs[i].table!)
            ?.data?.filter((r: Record<string, unknown>) => r.elevatoria_id === elev.id) ?? [];
        if (tabData.length === 0) {
          secos.push({ key: tabs[i].key, total: 0, preenchidos: 0, na: 0 });
          continue;
        }
        ...
        if (tabs[i].multi) {
          for (const row of tabData) {
            const fields = Object.keys(row as Record<string, unknown>).filter(
              (k) => !metaFields.includes(k),
            );
  ...
  const filtered = useMemo(() => {
    ...
    if (filtroImplantacao !== "TODAS") {
      if (filtroImplantacao === "operacional") {
        const idsComImplantacao = new Set(
          implantacoes.filter((i) => i.status === "operacional").map((i) => i.elevatoria_id),
        );
        list = list.filter((e) => !idsComImplantacao.has(e.id));
      } else {
        const idsFiltro = new Set(
          implantacoes.filter((i) => i.status === filtroImplantacao).map((i) => i.elevatoria_id),
        );
    ...
    list = [...list].sort((a, b) => {
      ...
      if (sortField === "completude") {
        ...
        const aVal = aImp
          ? IMPLANTACAO_STATUS_OPCOES.findIndex((o) => o.value === aImp.status)
          : -1;
        const bVal = bImp
  ...
  const excluirElevatoria = async (elev: Elevatoria) => {
    if (!confirm(`Tem certeza que deseja excluir "${elev.nome}"? Esta ação não pode ser desfeita.`))
      return;
  ...
  const criarElevatoria = async () => {
    ...
    if (data) {
      toast.success("Elevatória criada");
      navigate({ to: `/elevatorias/${data.id}` });
    }
  };
  ...
  const importarPlanilha = async (file: File) => {
    try {
      ...
      const sheetName = workbook.SheetNames.find(
        (n) =>
          n.toUpperCase().includes("RELAÇÃO DE ELEVATÓRIAS") || n.toUpperCase().includes("RELAÇÃO"),
      );
      if (!sheetName) {
        toast.error("Aba 'RELAÇÃO DE ELEVATÓRIAS' não encontrada");
        setLoading(false);
        return;
      }
      ...
      for (const row of dataRows) {
        ...
        if (
          ...
          nome.toUpperCase().includes("SUBTOTAL")
        )
          continue;
      ...
      if (allRows.length === 0) {
        toast.error("Nenhuma elevatória válida encontrada na planilha");
        setLoading(false);
        return;
      }
      ...
      const { data: insertedElevs, error: elevErr } = await supabase
        .from("elevatorias")
        .upsert(elevRecords, { onConflict: "nome", ignoreDuplicates: false })
        .select("id, nome");
      if (elevErr) {
        toast.error("Erro ao inserir elevatorias: " + elevErr.message);
        setLoading(false);
        return;
      }
      ...
      const upsertChild = async (
        ...
        const payload = records
          .filter((r) => nameToId.has(r.nome as string))
          .map((r) => {
        ...
        const { error } = await supabase
          .from(table)
          .upsert(payload, { onConflict: conflictCols, ignoreDuplicates: false });
      ...
      const equipFields = [
        ...
        "diametro_rotor_pol",
        "diametro_rotor_mm",
        "tipo_construtivo_elevatoria",
        "bomba_dreno",
        "ponta_eixo_motor",
        "sentido_montagem_motor",
        "flange",
        "forma_construtiva_bomba",
        "vazao_aproximada_m3h",
        "amt_aproximada",
        "capacidade_tratamento",
        "procedencia_mca",
      ];
      const eletFields = [
        "grupo",
        "fusivel_pc",
        "disjuntor_pc",
        "regulagem_rele_termico_bimetálico",
        "rele_tempo_delta_y",
        "rele_eletrodo_nivel",
        "monitor_corrente",
        "tamanho_fusivel_nh",
        ...
        "tag_painel",
        "tipo_acionamento",
        "fabricante_acionamento",
        "modelo_acionamento",
        "corrente_a_acionamento",
        "tag_acionamento",
        "retaguarda_liga",
        "retaguarda_desliga",
        "recalque_setpoint",
      ...
      await Promise.all([
        ...
        upsertChild(
          "elevatoria_eletrica_geral",
          eletGeralRecords,
          eletGeralFields,
          "elevatoria_id",
        ),
        upsertChild(
          "elevatoria_hidraulica",
          hidrRecords,
          ["succao", "recalque", "tronco", "distancia_ate_elev"],
          "elevatoria_id",
        ),
        upsertChild(
          "elevatoria_area_influencia",
          areaRecords,
      ...
      toast.error(
        "Erro ao processar planilha: " + (err instanceof Error ? err.message : "desconhecido"),
      );
      setLoading(false);
    }
  };
  ...
  const BadgeCompletude = ({
    ...
    return (
      ...
      >
        <span
          className={`h-2 w-2 rounded-full ${nivel === "bom" ? "bg-emerald-500" : nivel === "atencao" ? "bg-amber-500" : "bg-red-500"}`}
        />
        {percentual}%
      </span>
    );
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 p-3 md:p-6">
      ...
      {elevatorias.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-300 bg-white p-16 text-center dark:border-slate-600 dark:bg-slate-800">
          ...
          <h2 className="text-xl font-bold text-[#0b3a73] dark:text-white">
            Nenhuma elevatória cadastrada
          </h2>
          ...
          <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            >
              ...
              <div className="text-[11px] text-blue-500">
                de {kpis.total} elevatórias cadastradas
              </div>
            </div>
              ...
              <table className="min-w-[800px] w-full text-left text-[13px]">
                <thead className="sticky top-0 bg-[#eaf3fb] text-[12px] text-[#0b3a73] z-10 dark:bg-slate-700 dark:text-slate-200">
                  <tr>
                    ...
                    <th
                      className="whitespace-nowrap px-3 py-2.5 font-semibold cursor-pointer hover:text-[#1f7ad6]"
                      onClick={() => handleSort("implantacao")}
                    >
                      <span className="inline-flex items-center gap-1">
                        Implantação{" "}
                          ...
                          {permissoes.podeEditar && editandoTipo === elev.id ? (
                            <select
                              ...
                              onChange={async (e) => {
                                ...
                                setElevatorias((prev) =>
                                  prev.map((el) =>
                                    el.id === elev.id ? { ...el, tipo: newTipo } : el,
                                  ),
                                );
                                ...
                                if (error) {
                                  ...
                                  setElevatorias((prev) =>
                                    prev.map((el) =>
                                      el.id === elev.id ? { ...el, tipo: elev.tipo } : el,
                                    ),
                                  );
                                }
                              }}
                        ...
                        <td className="whitespace-nowrap px-3 py-2">
                          {permissoes.podeEditarMestres && editandoImplantacao === elev.id ? (
                                ...
                                setImplantacoes((prev) => {
                                  ...
                                  return [
                                    ...prev,
                                    {
                                      ...
                                      segmento: null,
                                      fase_atual: null,
                                      observacoes_inconformidades: null,
                                    } as ElevatoriaImplantacao,
                                  ];
                                });
                                const { error } = await supabase
                            ...
                            >
                              {imp ? (
                                <span
                                  className={`inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-[11px] font-semibold ${IMPLANTACAO_STATUS_CORES[imp.status] || ""}`}
                                >
                                  {IMPLANTACAO_STATUS_OPCOES.find((o) => o.value === imp.status)
                                    ?.label || imp.status}
                                </span>
                              ) : (
                          ...
                          {permissoes.podeEditar ? (
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                setElevatorias((prev) =>
                                  prev.map((el) =>
                                    el.id === elev.id ? { ...el, obs: newObs || null } : el,
                                  ),
                                );
                              }}
        ...
        <DialogContent className="max-w-3xl max-h-[80vh] flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            {registrosDialog !== null && (
              <ListaRegistros
                elevatoriaId={registrosDialog}
                permissoes={permissoes.permissoesRegistros}
              />
            )}
          </div>
        </DialogContent>
      ...
      <Dialog open={dialogImportar} onOpenChange={setDialogImportar}>
        <DialogContent className="max-w-md">
          ...
          <div className="py-4 text-sm text-slate-600 dark:text-slate-300">
            ...
            <p className="mb-3">
              A planilha deve conter as abas "RELAÇÃO DE ELEVATÓRIAS" e opcionalmente "ELEVATÓRIAS
              BXD2".
            </p>
            <input
              ref={fileInputRef}
