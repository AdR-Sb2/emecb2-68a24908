...
- Mapa de campo: busca "Buscar elevatoria..." (dropdown, Enter/selecao faz `flyTo` zoom 15 + marcador âmbar pulsante ~2,5s), tooltip no hover ("Nº. Nome" nas paradas), filtros Rota/Pendentes/Sem associação, legenda. Cores: paradas numeradas azul, pendentes verde, sem associação cinza. Clique numa pendente verde adiciona direto à rota.
- Pendentes (elevatorias marcadas p/ proxima rota): coluna `pendentes` JSONB (migration 00085). O save/leitura detecta a ausencia da coluna (`colunaPendentesDisponivel`, regex /pendentes/ no erro) e cai de forma transparente para payload sem `pendentes` + espelho em localStorage por id — o planejamento continua salvando; pendentes ficam per-usuario ate a migration rodar, depois sincronizam no banco (pra todos). Salvar e permitido com 0 paradas desde que haja pendentes. Campos: elevation (id/nome/planta/lat/lon) p/ duplicar nas novas rotas; mark no mapa verde.
