...
- Tabela de paradas (acordeão, uma elevatória por grupo): linha-cabeçalho com chevron (expandir/recolher, tudo expandido por padrão via `colapsadas`), badge "N OS", planta, ações subir/descer/remover da rota + botão "Adicionar OS". Cada O.S. vira uma sublinha: OS (input livre que auto-vincula O.S. existente por numero/texto via `vincularPorTexto(idx, osIdx, texto)`, badge laranja "Pendente criação" para novas), Tipo (select), Texto Breve, Status (Existente / Pendente criação / Nº livre / Sem O.S.), ações editar (abre dialog) e remover (confirmação). Toda elevatória tem no mínimo a barra "+ Adicionar OS".
