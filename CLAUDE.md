...
- Dialog de O.S. (`OsDialog`, props `alvo { idx, osIdx } | null`): modo adicionar (`osIdx === null`) ou editar (seeds os campos); 2 abas — "O.S. existente" (busca no backlogOS por nº/descricao/planta, vira origem "sistema") e "Nova O.S. (pendente)" (origem "nova"; tipo obrigatório; opcionais planta/equipamento/prioridade/texto breve/observacoes; nao cria nada no sistema). Handlers: `addOs(idx, os)` / `updateOs(idx, osIdx, patch)` / `removeOs(idx, osIdx)`; multiplas O.S. por parada em `Parada.oss: OsInfo[]` (helper `ossDeParada` dá fallback em `os` legado). Itens top-level: Salvar Planejamento, Otimizar Rota (vizinho mais proximo), Exportar (ExcelJS), Planejamentos (biblioteca: abrir/renomear/duplicar/excluir).
